...
def flatten_symmetry(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.drop(columns=["symmetry"]) #dropping the original whole dict thingymajig column
    return df

def clean_oxide_data(df: pd.DataFrame) -> pd.DataFrame:
    new_df = flatten_symmetry(df)
    
    numeric_cols = ["density", "energy_above_hull", "formation_energy_per_atom","nsites", "band_gap", "volume"]
    ...
    new_df["formation_energy_per_atom"] = new_df["formation_energy_per_atom"].mask(new_df["formation_energy_per_atom"] > 2)
...
def main():
    ...
    # sample = df["symmetry"].iloc[0]
    # print(type(sample), sample)
    # converted = string_to_dict(sample)
